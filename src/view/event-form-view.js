...
const createFormTemplate = ({card}) => {
  const {type, basePrice, dateFrom, dateTo, destination, offers} = card;
  const {name, description, photos} = destination;
  const timeGroup = getEventFieldGroupTime(dateFrom, dateTo);
  const offersAvailable = getEventAvailableOffers(offers);
